...
echo "Starting entrypoint for Celery beat..."
if ! wait_for_postgres "${PG_WAIT_TIMEOUT}"; then
  echo "Postgres did not become available - exiting" >&2
  exit 2
fi

echo "Postgres is available. Starting Celery beat..."
# Use exec so celery beat receives container signals
exec celery -A "${CELERY_APP}" beat --loglevel="${CELERY_LOGLEVEL}" --scheduler django_celery_beat.schedulers:DatabaseScheduler
